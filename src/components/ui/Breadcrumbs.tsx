        ...
        <nav className="flex items-center space-x-2 text-xs md:text-sm text-gray-500 py-3 px-4 bg-gray-50/50 rounded-xl border border-gray-100 max-w-7xl mx-auto my-4 shadow-sm" aria-label="Breadcrumb">
            <ol className="inline-flex items-center space-x-1.5 md:space-x-2">
                <li className="inline-flex items-center">
                    <Link href="/" className="inline-flex items-center gap-1.5 text-gray-500 hover:text-primary-600 transition-colors font-medium">
                ...
                {items.map((item, index) => {
                    ...
                    return (
                        <li key={index} className="inline-flex items-center">
                            ...
                            {item.url && !isLast ? (
                                <Link href={item.url} className="text-gray-500 hover:text-primary-600 transition-colors font-medium">
                                    {item.label}
                                </Link>
                            ) : (
